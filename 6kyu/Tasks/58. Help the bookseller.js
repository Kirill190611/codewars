//Task: A bookseller has lots of books classified in 26 categories labeled A, B, C, ..., Z. Each book has a code of at least 3 characters. The 1st character of a code is a capital letter which defines the book category.
//
...
const stockList = (books, categories) => {
    ...
    return categories.map(el => {
        const result = books.reduce((a, b) => a + (b.charAt(0) === el ? +b.split(' ')[1] : 0), 0)
        return `(${el} : ${result})`
    }).join(' - ')
}