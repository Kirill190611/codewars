...
function wantedWords(n, m, forbid_let) {
    ...
    return wordList.filter((ele) => {
        if (forbid_let.some(e => ele.includes(e))) return false;
        let vowelsCount = [...ele].reduce((count, char) => count + vowels.has(char), 0);
        let consonantsCount = ele.length - vowelsCount;
        return vowelsCount === n && consonantsCount === m;
    })
}