...
//Solution:
const encode = text => [...text].map(el => [...el.charCodeAt().toString(2).padStart(8, '0')].map(e => e.repeat(3)).join('')).join('')

const decode = bits => bits.match(/.{3}/g).reduce((p, c) => p + [...c].sort()[1],'').match(/.{8}/g).map(el => String.fromCharCode(parseInt(el, 2))).join('')