//Task: Given a list of positive integers, determine the minimum non-negative integer that needs to be inserted so that the sum of all elements becomes a prime number.
//
// Notes
...
const minimumNumber = numbers => {
    ...
    let digit = numbers.reduce((a, b) => a + b)
    while (!isPrime(digit)) {
        digit++
    }
    return digit - numbers.reduce((a, b) => a + b)
}